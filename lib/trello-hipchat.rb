#TrelloHipChat
#Parse activity feed events in a HipChat room
...
opts = Trollop::options do
  opt :hipchat_token, "HipChat API Token", :required => true, :type => String
  opt :hipchat_room, "HipChat Room Name", :required => true, :type => String
  opt :hipchat_username_to_display, "HipChat Room Name", :required => true, :type => String
...
hipchat = HipChat::Client.new(opts[:hipchat_token])
hipchat_username_to_display = opts[:hipchat_username_to_display]
hipchat_room = opts[:hipchat_room]
...
while true do
  ...
  if @last_date
    ...
    if i >= 0
      ...
      while i >= 0
        message_to_post = (TrelloHipChat.parse(parsed_respond[i], parsed_cards))
        hipchat[hipchat_room].send(hipchat_username_to_display, message_to_post, :message_format => 'text')
